...
const char* blur_cs[] =
{
	...
	"void main(void)                                                                         \n"
	"{                                                                                       \n"
	"	int locx = int(gl_GlobalInvocationID.x) - 3;                                         \n"
	"	int locy = int(gl_GlobalInvocationID.y) - 3;                                         \n"
	"                                                                                        \n"
	"	vec4 sum = vec4(0);                                                                  \n"
	"	for(int i = 0; i < 7; ++i)                                                           \n"
	"	{                                                                                    \n"
	"		int locx2 = clamp(locx + i, 0, IMAGE_WIDTH - 1);                                 \n"
	"		for(int j = 0; j < 7; ++j)                                                       \n"
	"		{                                                                                \n"
	"			int locy2 = clamp(locy + j, 0, IMAGE_HEIGHT - 1);                            \n"
	"			sum += imageLoad(input_image, ivec2(locx2, locy2)) * kernal[i] * kernal[j];  \n"
	"		}                                                                                \n"
	"	}                                                                                    \n"
	"                                                                                        \n"
...
void My_Init()
{
	...
	glShaderSource(cs, 1, blur_cs, NULL);
	glCompileShader(cs);
	glAttachShader(program_blur, cs);
...
void My_Display()
{
    ...
    glDispatchCompute(IMAGE_WIDTH / 10, IMAGE_HEIGHT / 10, 1);
	glBindTexture(GL_TEXTURE_2D, texture_out);
